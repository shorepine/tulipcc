...
# Fill the BG with noise
# block_size is the number of pixels in x and y each block of noise should be. Start with 1.
# c0, c1, c2, c3 are pal_idx colors (0-255) to use for the noise.
# If only 1 color is passed, a 2nd color will be randomly chosen.
# Otherwise, if 2,3, or 4 colors are passed, they will all be used.
tulip.bg_noise(block_size, c0, c1, c2, c3)
tulip.bg_noise(1) # fill screen with small noise in random colors
tulip.bg_noise(4, 0, 255) # fill screen with 4x4 pixel blocks of black and white noise
tulip.bg_noise(2, 206, 207, 226, 227) # fill screen with 2x2 pixel blocks with subtle pink variations
