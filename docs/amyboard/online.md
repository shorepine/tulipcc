...
When AMYboard starts up, either on web or hardware, it sets up whatever patches are set to each channel (using data stored in a sketch) and then runs a "sketch" set up in `sketch.py`. This is a Python program that runs setup code (in the top level of the code file) and then calls `loop(tick)` periodically -- every 32nd note of the sequencer, starting on a bar downbeat, which works out to an interval of `7500 / tempo` ms (69 ms at the default tempo of 108). `tick` is the AMY sequencer's absolute tick count -- the same clock `sequencer.AMYSequence` events and `amy.send(ticks=...)` use, so you can pass it straight back without converting. For 32nd-note counting, divide: `step = tick // amyboard.TICKS_PER_STEP` gives the bar-locked step index, where `step % 32 == 0` is a downbeat. The argument is required -- a zero-argument `loop()` raises a `TypeError` on every call, and the sketch loop stops itself after 8 consecutive failures. See [How often is `loop()` called?](faq.md#how-often-is-loop-called) if you need a faster callback. 
