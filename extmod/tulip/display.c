...
void display_run() {
    ...
    int64_t free_time = 0;
    int64_t tic0 = get_time_us();
    uint16_t loop_count =0;
    while(1)  { 
        int64_t tic1 = get_time_us();
        take_semaphore();
        free_time += (get_time_us() - tic1);
        if(loop_count++ >= 100) {
            reported_fps = 1000000.0f / ((get_time_us() - tic0) / loop_count);
            reported_gpu_usage = ((float)(bounce_time/bounce_count) / (1000000.0f / ((H_RES*V_RES / BOUNCE_BUFFER_SIZE_PX) * (reported_fps))))*100.0f;
            if(desync) {
                fprintf(stderr,"Desync warning: past %d frames: %2.2f FPS. free time %llduS. bounce time per is %llduS, %2.2f%% of max (%duS). bounce_count %d\n", 
                    loop_count,
                    (double)reported_fps, 
                    free_time / loop_count,
                    bounce_time / bounce_count,
                    (double)reported_gpu_usage,
                    (int) (1000000.0f / ((H_RES*V_RES / BOUNCE_BUFFER_SIZE_PX) * (reported_fps))),
                    bounce_count
                );
            }
            bounce_count = 1;
