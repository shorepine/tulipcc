...
print("13. scan_i2c() identifies what is on the bus")
use({0x3C: "other", 0x49: 5740, 0x58: "other"})
rows = amyboard.scan_i2c(verbose=True)
check("addresses", [r[0] for r in rows], [0x3C, 0x49, 0x58])
check("types", [r[1] for r in rows], [None, "adafruit_ano", None])
check("OLED named", rows[0][2], "OLED display")
check("ANO product reported", rows[1][2], "seesaw board, firmware product 5740")
check("CV DAC flagged unprobed", "not probed" in rows[2][2], True)

print("14. no encoder hardware at all")
use({})
enc = amyboard.encoder()
