...
// Micropython will send utf-8 codes through the REPL, here we convert them back to cp437 codes
uint8_t unicode_byte_to_cp437(uint8_t x) {
    static const uint16_t uc437[] = {  /*  CP437(U.S.) to Unicode conversion table */
    ...
    uint16_t find = 0x0000 | x;
    for(uint8_t i=0;i<128;i++) {
        if(uc437[i]==find) {
            fprintf(stderr, "found %d from %d\n", i, x);
            return i+128;
        }
    }
    fprintf(stderr, "coulnd't find %d\n", x);
    return 32; //empty
}
...
void display_tfb_str(unsigned char*str, uint16_t len, uint8_t format, uint8_t fg_color, uint8_t bg_color) {
    ...
    for(uint16_t i=0;i<len;i++) {
        if(str[i] == 8)  { // backspace , go backwards (don't delete)
            ...
            // eat it, mangle the next one to cp437
            str[i+1] = unicode_byte_to_cp437(str[i+1]);
            i++;
        } else if(str[i] == 0xc3) {
            str[i+1] = unicode_byte_to_cp437(str[i+1]+64);
            i++;
        }
        if(str[i] == 27) { // ANSI
