...
def generate_patches():
    ...
    output += "    var groups = [\n"
    output += "      { label: \"AMYboard\", start: 0, end: Math.min(patches.length, 128) },\n"
    output += "      { label: \"DX7\", start: 128, end: patches.length },\n"
    output += "    ];\n"
    output += "    for (var g = 0; g < groups.length; g++) {\n"
    output += "      var grp = groups[g];\n"
    output += "      if (grp.start >= patches.length) continue;\n"
    output += "      var optgroup = document.createElement(\"optgroup\");\n"
    output += "      optgroup.label = grp.label;\n"
    output += "      for (var index = grp.start; index < grp.end; index++) {\n"
    output += "        var option = document.createElement(\"option\");\n"
    output += "        option.value = String(index);\n"
    output += "        option.textContent = patches[index];\n"
    output += "        optgroup.appendChild(option);\n"
    output += "      }\n"
    output += "      select.appendChild(optgroup);\n"
    output += "    }\n"
    output += "  };\n"
    output += "})();\n"
