...
def generate_drum_presets_js():
    """Write static/drum_presets.generated.js from AMY's gamma9001 manifest.

    Maps PCM preset number -> {name, frames, root} for the Gamma9001 drum
    banks (presets 0-18 TR-808 + 256-391). The editor's drum kit knob view
    uses names for row labels and frames to convert the Offset knob into an
    AMY phase (start_frame / 2^23)."""
    manifest_path = os.path.join(AMY_DOCS, "gamma9001", "samples", "manifest.json")
    ...
    for entry in manifest:
        ...
        if not isinstance(preset, int):
            continue
        presets[preset] = {
            "name": entry.get("name", "preset %d" % preset),
            "frames": entry.get("preset_frames", entry.get("frames", 0)),
            "root": entry.get("root", 60),
        }
    out = os.path.join(STATIC_DIR, "drum_presets.generated.js")
    with open(out, "w", encoding="utf-8") as f:
        f.write("// AUTO-GENERATED by dev.py from amy/docs/gamma9001/samples/manifest.json"
                " — do not edit by hand.\n")
        f.write("window.AMY_DRUM_PRESETS = ")
        f.write(json.dumps({str(k): presets[k] for k in sorted(presets)}, indent=1, sort_keys=True))
        f.write(";\n")
