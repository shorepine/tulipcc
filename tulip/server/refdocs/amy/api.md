...
| `amy_external_cv_output_hook` | `void (uint8_t channel, float volts)` | — | A synth with `note_output='CV_GATE,...'` is putting `volts` on host control output `channel` (0 .. `AMY_MAX_CV_OUT`-1). AMY has no opinion about what a control output physically is — a DAC channel, a GPIO driven to 0 or 5 V — which is what makes using a CV output as a gate free. Called from the render thread as the note event is applied; keep it to a register write or a mailbox post. See [note_outputs.md](note_outputs.md). |
...
| `ig`   | `cv_trigger` | `cv_trigger` | uint,uint | Configure external CV event triggering: Gate CV, trigger threshold, reset threshold, pitch CV, pitch scale, pitch offset, wire command template, %v gets pitch value. The pitch is `midi_note_for_logfreq(offset + scale * volts)`, i.e. scale and offset are in OCTAVES with 0 at 440 Hz (note 69): `1,0` is 1V/oct with A4 at 0 V, and 1V/oct with C1 at 0 V is `1,-3.75`. |
| `iG`   | `note_output` | `note_output` | str | Send this synth's NOTE EVENTS to a control output IN ADDITION to its own oscillators. `'<mode>[,<arg>...]'`, the mode being the constant interpolated: `'%d,<pitch_cv>,<gate_cv>[,<vel_cv>[,<scale>[,<offset>[,<gate_volts>]]]]' % amy.NOTE_OUTPUT_CV_GATE` for 1V/oct plus a gate, `'%d,<channel 1..16>[,<forward_midi_in>]' % amy.NOTE_OUTPUT_MIDI_OUT` for MIDI, `'%d' % amy.NOTE_OUTPUT_OFF` to stop. A synth that should be silent inside AMY is simply one with no voices. See [note_outputs.md](note_outputs.md). |
