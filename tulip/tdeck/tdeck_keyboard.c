...
void watch_trackball(void *params)
{
    ...
    while (true)
    {
        if (xQueueReceive(interruptQueue, &pinNumber, portMAX_DELAY))
        {
            ...
            //if(pinNumber == TDECK_TOUCH_INT) {
            //  if(gpio_get_level(pinNumber)) {
            //    i2c_master_read_from_device(I2C_NUM_0, TDECK_TOUCH_ADDR_0, raw_data, 40, pdMS_TO_TICKS(TIMEOUT_MS));
            //    fprintf(stderr, "got touch, lvl %d %d %d %d %d %d %d\n", gpio_get_level(pinNumber), raw_data[0], raw_data[1], raw_data[2], raw_data[3], raw_data[4], raw_data[5]);
            //  }
...
void run_tdeck_keyboard() {
  ...
  ESP_ERROR_CHECK(i2c_driver_install(I2C_NUM_0, I2C_MODE_MASTER, 0, 0, 0));
    ...
    gpio_set_direction(TDECK_TOUCH_INT, GPIO_MODE_INPUT);
    gpio_pulldown_dis(TDECK_TOUCH_INT);
    gpio_pullup_en(TDECK_TOUCH_INT);
    gpio_set_intr_type(TDECK_TOUCH_INT, GPIO_INTR_POSEDGE);
...
//    gpio_install_isr_service(0);
    ...
    //gpio_isr_handler_add(TDECK_TOUCH_INT, gpio_interrupt_handler, (void *)TDECK_TOUCH_INT);
  ...
  while (1) {
    ...
    if(rx_data[0]>0) {
      if(rx_data[0]==4)  { 
        ...
        send_key_to_micropython(24); // ctrl x
      } else {
